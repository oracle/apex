...
CREATE OR REPLACE PACKAGE BODY DBMS_GVT IS
 ...
 -----------------------------------------------------------------------------
  ...
  BEGIN
    RETURN '26.4.0 (2026-09-22T20:31:18.288120722Z, build: abf4b9452)';
  END GET_VERSION;
  ...
  FUNCTION PROPERTIES_LATERAL_STRING_AS_CLOB (
    ...
    ELEMENT_TABLE_COLUMN_NAME          VARCHARLIST_TABLE; 
    ELEMENT_TABLE_PROPERTY_NAME        VARCHARLIST_TABLE; 
    ELEMENT_TO_KEY_LIST_TABLE          VARCHARLIST_TABLE; 
    ...
    OBJECT_TO_ELEMENTS                 VARCHARLIST_TABLE; 
    ...
    INNER_PROPS_LABELS_STRING          CLOB;
    EDGE_INNER_PROJECTION_STRING       CLOB;
    INNER_QUERY_STRING                 CLOB;
    OUTER_SELECT_STRING                CLOB;
    SRC_OUTER_JOIN_STRING              CLOB;
    DEST_OUTER_JOIN_STRING             CLOB;
    SRC_PROJECTED_COL_ALIAS            VARCHAR2(M_VCSIZ_4K);
    DEST_PROJECTED_COL_ALIAS           VARCHAR2(M_VCSIZ_4K);
  BEGIN
    ...
    FOR IDX1 IN 1..DB_TABLE_NAME_LIST.COUNT LOOP
      FOR IDX6 IN 1..OBJECT_TO_ELEMENTS(DB_TABLE_NAME_LIST(IDX1)).COUNT LOOP
          ...
          IF IDX1 = IDX2 THEN
            FOR IDX4 IN 1..ELEMENT_TABLE_PROPERTY_NAME(ELEMENT_NAME).COUNT LOOP
              ...
              IF COLUMN_EXPRESSION IS NULL THEN
                INNER_PROPS_LABELS_STRING := INNER_PROPS_LABELS_STRING
                                             || 'q''['
        ...
        IF ELEMENT_TYPE = 'EDGE' THEN
          EDGE_INNER_PROJECTION_STRING := '';
          FOR IDX4 IN 1..SRC_EDGE_COL_NAME_TABLE(ELEMENT_NAME).COUNT LOOP
            IF SRC_EDGE_COL_NAME_TABLE(ELEMENT_NAME).COUNT = 1 THEN
              SRC_PROJECTED_COL_ALIAS := '__SRC_COL';
            ELSE
              SRC_PROJECTED_COL_ALIAS := '__SRC_COL_' || IDX4;
            END IF;
            EDGE_INNER_PROJECTION_STRING := EDGE_INNER_PROJECTION_STRING
                                            || ', X."'
          ...
          FOR IDX4 IN 1..DEST_EDGE_COL_NAME_TABLE(ELEMENT_NAME).COUNT LOOP
            IF DEST_EDGE_COL_NAME_TABLE(ELEMENT_NAME).COUNT = 1 THEN
              DEST_PROJECTED_COL_ALIAS := '__DST_COL';
            ELSE
              DEST_PROJECTED_COL_ALIAS := '__DST_COL_' || IDX4;
            END IF;
            EDGE_INNER_PROJECTION_STRING := EDGE_INNER_PROJECTION_STRING
                                            || ', X."'
          ...
          OUTER_SELECT_STRING := 'SELECT p.properties, p.labels, q''['
          ...
          FOR IDX4 IN 1..SRC_EDGE_COL_NAME_TABLE(ELEMENT_NAME).COUNT LOOP
            ...
            ELSE
              SRC_PROJECTED_COL_ALIAS := '__SRC_COL_' || IDX4;
            END IF;
            SRC_OUTER_JOIN_STRING := SRC_OUTER_JOIN_STRING
                                     || 'p."'
                                     || SRC_PROJECTED_COL_ALIAS
                                     || '" = src_table."'
          ...
          FOR IDX4 IN 1..DEST_EDGE_COL_NAME_TABLE(ELEMENT_NAME).COUNT LOOP
            ...
            ELSE
              DEST_PROJECTED_COL_ALIAS := '__DST_COL_' || IDX4;
            END IF;
            DEST_OUTER_JOIN_STRING := DEST_OUTER_JOIN_STRING
                                      || 'p."'
                                      || DEST_PROJECTED_COL_ALIAS
                                      || '" = dst_table."'
          ...
          LATERAL_QUERY_STRING := OUTER_SELECT_STRING
                                  || 'FROM ('
                                  || INNER_QUERY_STRING
                                  || ') p '
                                  || 'JOIN "'
        ...
        ELSE                   
          ...
          LATERAL_QUERY_STRING := 'SELECT p.properties, p.labels FROM ('
                                  || INNER_QUERY_STRING
                                  || ') p';
        END IF;
 ...
 FUNCTION BUILD_JSON_USING_JSON_ARRAY(
  ...
  BEGIN 
    ...
    IF (VERTEX_TABLE.get_Size != 0) THEN
      ...
      SUB_QUERY_STRING := '
          SELECT
            JSON_OBJECT (''id'' VALUE JSON_VALUE(VT.V_ID, ''$.ELEM_TABLE'') || JSON_QUERY(VT.V_ID, ''$.KEY_VALUE''),
            ''original_id'' VALUE VT.V_ID,
    ...
    IF (EDGE_TABLE.get_Size != 0) THEN
      ...
      QUERY_STRING := 'WITH EDGES AS (
            SELECT
              JSON_OBJECT(''id'' VALUE JSON_VALUE(ET.E_ID, ''$.ELEM_TABLE'') || JSON_QUERY(ET.E_ID, ''$.KEY_VALUE''),
              ''original_id'' VALUE ET.E_ID,
...
CREATE OR REPLACE FUNCTION ORA_SQLGRAPH_TO_JSON (
  ...
  L_FLAG                                  NUMBER;
  L_JSON                                  JSON;
  TYPE JSON_ARRAY_TAB IS TABLE OF JSON_ARRAY_T INDEX BY PLS_INTEGER;
  TYPE ELEMENT_KIND_BY_COLUMN_TAB IS TABLE OF VARCHAR2(M_VCSIZ_4K) INDEX BY PLS_INTEGER;
  COLUMN_VALUES                           JSON_ARRAY_TAB;
  ELEMENT_KIND_BY_COLUMN                  ELEMENT_KIND_BY_COLUMN_TAB;
...
BEGIN
  ...
  FOR POS IN 1 .. L_COLS LOOP
    CASE TAB_REC (POS).COL_TYPE
      WHEN 119 THEN
        ...
        COLUMN_VALUES(POS) := JSON_ARRAY_T();
      ELSE
        NULL;
    END CASE;
  END LOOP;
  ...
  LOOP
    ...
    IF FETCH_ROWS_INTEGER > 0 THEN
      ...
      L_RENDER_ROW := ((COUNTER > PAGE_START
      ...
      FOR POS IN 1 .. L_COLS LOOP
        IF TAB_REC(POS).COL_TYPE = 119 THEN
          ...
          IF L_RENDER_ROW THEN
            COLUMN_VALUES(POS).APPEND(L_JSON);
          END IF;
          IF L_JSON IS NOT NULL
          AND JSON_EXISTS(L_JSON, '$.ELEM_TABLE')
          AND JSON_EXISTS(L_JSON, '$.GRAPH_OWNER')
          AND JSON_EXISTS(L_JSON, '$.GRAPH_NAME')
          AND JSON_EXISTS(L_JSON, '$.KEY_VALUE') THEN
            ...
            ELSIF GRAPHNAME != JSON_VALUE(L_JSON, '$.GRAPH_NAME')
              OR GRAPHOWNER != JSON_VALUE(L_JSON, '$.GRAPH_OWNER') THEN
            ...
            END IF;
            ELEMENT_KIND_BY_COLUMN(POS) := ELEMENT_NAME;
          END IF;
        END IF;
      END LOOP;
  ...
  FOR POS IN 1 .. L_COLS LOOP
    IF ELEMENT_KIND_BY_COLUMN.EXISTS(POS) THEN
      IF ELEMENT_KIND_BY_COLUMN(POS) = 'VERTEX' THEN
  ...
  IF P1 > 0 THEN
    FOR ROW_INDEX IN 0 .. P1 - 1 LOOP
      FOR POS IN 1 .. L_COLS LOOP
        IF ELEMENT_KIND_BY_COLUMN.EXISTS(POS) THEN
          IF ELEMENT_KIND_BY_COLUMN(POS) = 'VERTEX' THEN
            VERTEX_TABLE.APPEND(COLUMN_VALUES(POS).GET(ROW_INDEX));
          ELSE
            EDGE_TABLE.APPEND(COLUMN_VALUES(POS).GET(ROW_INDEX));
          END IF;
        END IF;
      END LOOP;
    END LOOP;
